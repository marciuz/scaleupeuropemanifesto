...
class mysqli_vmsql {
	...
	*/
        ...
        try{
           ...
           if(!is_object($this->link_db)){
               throw new DbException(
                       "Connection error: is MySQL running? Otherwise please check your conf file.", Edfx_Enum::E_MYSQL_DOWN);
           }
        }
        catch( DbException $e){
	...
	public function query($sql){
        ...
        try{
            if($obj===false){
                throw new DbException(
                        mysqli_error($this->link_db), 
                        Edfx_Enum::E_MYSQL_GENERIC);
            }
        }
        catch(DbException $e){
	 ...
	 * @param string $error prende l'errore
	 * @param int $errono prende in numero dell'errore
	 * @return bool Esito della query
	 */
	public function query_try($sql, $secure_mode=true, &$error=false, &$errno=false){
	...
	*/
     ...
     * @return array
     */
    public function get($sql, $type='', $reverse=false, $classname=null){
        ...
        switch($type){
            ...
            case 'object' : $mat=$this->fetch_object_all($q, $classname);
            break;
	...
	*/
	public function fetch_object_all(&$res, $class_name=null){
		...
		if(is_object($res)){
			...
			$matrice=array();
			while($RS= $this->fetch_object($res, $class_name)) $matrice[]=$RS;
			$this->free_result($res);
	...
	*/
	 ...
	 * @param resource $this->link_db
	 */
	public function commit($reset_autocommit=true){
    ...
    public function end_transaction(){
		...
		mysqli_autocommit($this->link_db, TRUE);
	}
	...
	protected function close(){
		...
		return ($this->connected && is_object($this->link_db)) ? mysqli_close($this->link_db) : null;
	}
	...
	protected function db_error_log($obj){
	    ...
	    if($fp=@fopen(Edfx_Enum::FILE_ERRORLOG_DB,"a")){
            $towrite='';
            ...
            if(is_array($obj->errors)){
                ...
                foreach($obj->errors as $e){
                    ...
                    // prende il tipo query (SELECT , INSERT, UPDATE, DELETE) 
...
class Vmsql extends mysqli_vmsql{
